    ...
    controllers: {
        columns: modules.Controller.inherit(function() {
            ...
            var createColumn = function(that, columnOptions, userStateColumnOptions, bandColumn) {
                var calculatedColumnOptions, isDefaultCommandColumn, commonColumnOptions = {};
                if (columnOptions) {
            ...
            var columnOptionCore = function(that, column, optionName, value, notFireEvent) {
                var prevValue, optionSetter, columns, changeType, fullOptionName, optionGetter = dataCoreUtils.compileGetter(optionName),
            ...
            var mergeColumns = function(columns, commandColumns, needToExtend) {
                var i, column, commandColumnIndex, result = columns.length ? (needToExtend && commandColumns || columns).slice().map(function(column) {
            ...
            return {
                ...
                getFilteringColumns: function() {
                    return this.getColumns().filter(function(item) {
                        return (item.dataField || item.name) && item.allowFiltering
                    }).map(function(item) {
                ...
                columnOption: function(identifier, option, value, notFireEvent) {
                    var i, needUpdateIndexes, column, that = this,
                        ...
                        columns = identifier < 0 || "command" === identifierOptionName ? that._columns.concat(that._commandColumns) : that._columns;
