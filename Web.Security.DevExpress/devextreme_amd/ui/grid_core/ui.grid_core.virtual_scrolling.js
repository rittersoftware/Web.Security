        ...
        _updateContent: function(tableElement, change) {
            ...
            if ("append" === changeType || "prepend" === changeType) {
                ...
                if (!that.option("legacyRendering") && 1 === $tBodies.length) {
                    getBodies(contentTable)["append" === changeType ? "append" : "prepend"]($tBodies.children())
                } else {
                ...
                var members = {
                    ...
                    _updateItemsCore: function(change) {
                        ...
                        if (rowsScrollController) {
                            ...
                            var isRefresh = "refresh" === change.changeType;
                    ...
                    _applyChange: function(change) {
                        ...
                        if (removeCount) {
                            for (var i = 0; i < removeCount + 1; i++) {
                                var item = that._items["prepend" === changeType ? that._items.length - 1 - i : i];
                                if (item && "data" !== item.rowType && "group" !== item.rowType) {
                                    removeCount++
                                }
                            }
