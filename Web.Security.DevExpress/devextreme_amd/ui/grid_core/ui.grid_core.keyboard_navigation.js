...
var KeyboardNavigationController = core.ViewController.inherit({
    ...
    _updateFocus: function() {
        var that = this,
            ...
            $cellEditingCell = that._isCellEditMode() ? $cell : void 0;
            ...
            if ($cell && $cell.length > 0) {
                setTimeout(function() {
                    if ($cell.is("td") || $cell.hasClass(that.addWidgetPrefix(EDIT_FORM_ITEM_CLASS))) {
                        if (that.getController("editorFactory").focus($cellEditingCell) || $cellEditingCell) {
                            that._focus($cell)
                        } else {
    ...
    _clickHandler: function(e) {
        var columnIndex, column, event = e.event,
        ...
        each(that._focusedViews, function(index, view) {
            if (view) {
                view.renderCompleted.add(function(e) {
                    ...
                    var isPartialUpdate = e && "update" === e.changeType;
    ...
    _focus: function($cell, disableFocus, isInteractiveElement) {
        ...
        var $focusElement, $focusedCell = this._getFocusedCell(),
