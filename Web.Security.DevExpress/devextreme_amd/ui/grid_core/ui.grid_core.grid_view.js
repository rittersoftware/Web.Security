...
var ResizingController = modules.ViewController.inherit({
    ...
    _synchronizeColumns: function() {
        var resetBestFitMode, focusedElement, selectionRange, that = this,
    ...
    _correctColumnWidths: function(resultWidths, visibleColumns) {
        ...
        if (!hasAutoWidth && resultWidths.length) {
            ...
            if (totalWidth < contentWidth) {
                ...
                var hasResizableColumns = visibleColumns.some(function(column) {
                    return column && !column.command && !column.fixed && false !== column.allowResizing
                });
                while (lastColumnIndex >= 0 && visibleColumns[lastColumnIndex] && (visibleColumns[lastColumnIndex].command || resultWidths[lastColumnIndex] === HIDDEN_COLUMNS_WIDTH || visibleColumns[lastColumnIndex].fixed || hasResizableColumns && false === visibleColumns[lastColumnIndex].allowResizing)) {
                    lastColumnIndex--
                }
