...
exports.ColumnsView = modules.View.inherit(columnStateMixin).inherit({
    ...
    _createTable: function(columns, isAppend) {
        var that = this,
            $table = $("<table>").addClass(that.addWidgetPrefix(TABLE_CLASS)).addClass(that.addWidgetPrefix(TABLE_FIXED_CLASS));
        if (columns && !isAppend) {
    ...
    _renderTable: function(options) {
        ...
        $table = that._createTable(options.columns, "append" === changeType || "prepend" === changeType);
