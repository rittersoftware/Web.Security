...
extend(ChartTracker.prototype, baseTrackerPrototype, {
    ...
    _gestureChange: function(gestureParams) {
        ...
        if (1 === startGesture.touches && Math.abs(startGesture.center - gestureParams.center) < SCROLL_THRESHOLD) {
    ...
    _handleStartScrollTimeout: function(e) {
        var that = this;
        if (that._gestureStartTimeStamp && 1 === that._startGesture.touches && e.timeStamp - that._gestureStartTimeStamp < TOUCHSTART_TIMEOUT && that._startGesture.distance >= SCROLL_THRESHOLD) {
            that._stopEvent(e);
            that._pointerOut();
