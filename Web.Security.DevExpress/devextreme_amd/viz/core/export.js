...
function _print(data, backgroundColor) {
    var svg, vizWindow = windowUtils.openWindow();
    if (!vizWindow) {
        return
    }
        ...
        that._renderer.root.on(pointerEvents.up + ".export", function(e) {
            ...
            if ("button" === elementType) {
                ...
                if ("printing" === elementType) {
                    that.hide();
                    _print(that._svgMethod(), options.backgroundColor);
                    that.show();
                    that._hideList()
                } else {
                    if ("exporting" === elementType) {
                        exportOptions = extend({}, options.exportOptions, {
...
exports.plugin = {
    ...
    members: {
        ...
        print: function() {
            _print(this.svg(), this._getOption("export").backgroundColor)
        }
    },
